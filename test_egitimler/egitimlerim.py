...
class test_egitimler:   
        ...
        #kurum seçimi arama butonu İstanbul Kodluyor seçimi
            ...
            kurum_btn.click()
            istkod = WebDriverWait(self.driver,5).until(ec.visibility_of_element_located((By.XPATH, gc.ISTKOD)))
            istkod.click()            
            sleep(5)
            kurum_btn.clear()
        #kurum seçimi filtreleme çubuğuna karakter girişi yapınız.
            kurum_btn = WebDriverWait(self.driver,5).until(ec.visibility_of_element_located((By.XPATH, gc.KURUM)))
            kurum_btn.click()
            
egitim = test_egitimler()
egitim.test_egitim_var()